...
export async function create(
  ...
  try {
    const { name, action_type, subscribers, action_config } =
      req.body as CreatePipelineInput;

    if (!name || !action_type || !subscribers) {
      res.status(400).json({
        error: 'Missing required fields: name, action_type, subscribers',
      });
      return;
    }
    ...
    if (!['transform', 'filter', 'enrich'].includes(action_type)) {
      res.status(400).json({
        error: 'action_type must be one of: transform, filter, enrich',
      });
      return;
    }
